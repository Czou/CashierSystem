...
/**
 * 模块描述：软件版本更新管理 2014-10-15 下午5:35:24 Write by LILIN
 */
public class CheckVersionManager {
	...
	public static void checkVersion(Activity mContext, boolean isLoading) {
		CheckVersionManager.mContext = mContext;
		CheckVersionManager.isLoading = isLoading;
		if (BaseUtils.isNetworkAvailable(mContext)) {
			ConnectManager.getInstance().getAppUpdateInfo(
					checkVersionAjaxCallBack);
			if (isLoading) {
				C.openProgressDialog(mContext, onKeyListener,
						R.string.checkVersion);
			}
		} else {
			C.showDialogAlert("当前网络不可用", mContext);
		}
	}
		...
		public void onSuccess(String t) {
			...
			try {
				if (BaseUtils.IsNotEmpty(t)
					...
					String app_version = JSONParser.getStringFromJsonString(
							"app_version", data);
					if (BaseUtils.IsNotEmpty(app_version)) {
						AppVersion appVersion = (AppVersion) JSONParser
								.JSON2Object(app_version, AppVersion.class);
						...
						// 服务器版本
						int updateVersionCode = Integer
								.parseInt(appVersion.version_num);
						LG.i(getClass(), "versionName:" + appVersion.version);
						if (versionCode < updateVersionCode)// 有版本更新
						{
							Intent intent = new Intent(mContext,
									TipUpdateVersionAtivity.class);
							intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
							intent.putExtra("function",
									appVersion.version_content);
							// intent.putExtra("must_update",
							// appVersion.must_update);
							intent.putExtra("download_url", appVersion.down_url);
							mContext.startActivity(intent);
						} else {
							if (isLoading) {
								C.showShort(mContext
										.getString(R.string.noFindNewVersion),
										mContext);
							}
						}
					}
				} else {
					String alert = JSONParser.getStringFromJsonString(
							"error_desc", t) + "";
					if (isLoading) {
